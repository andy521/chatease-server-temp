...
(function(abjchat) {
	...
	abjchat.api.displayError = function(message, config) {
	...
	core.controller = function(model, view) {
			...
			_websocket.onmessage = function(e) {
				...
				switch (data.raw) {
					case 'ident':
						...
						_this.dispatchEvent(events.ABJCHAT_INDENT, data);
						break;
					case 'message':
					...
					case 'error':
						...
						if (explain) 
							view.show(explain);
						_this.dispatchEvent(events.ABJCHAT_ERROR, data);
						break;
					default:
		...
		function _modelStateHandler(e) {
			switch (e.state) {
				...
				case states.ERROR:
					view.show('聊天室异常！');
					_this.dispatchEvent(events.ABJCHAT_ERROR, { message: 'Chat room error!', channelId: model.id });
					break;
				default:
					_this.dispatchEvent(events.ABJCHAT_ERROR, { message: 'Unknown model state!', state: e.state });
					break;
			}
		}
...
(function(abjchat) {
	...
	var embed = abjchat.embed = function(api) {
		...
		_this.errorScreen = function(message) {
			...
			_errorOccurred = true;
			abjchat.api.displayError(message, _config);
		};
